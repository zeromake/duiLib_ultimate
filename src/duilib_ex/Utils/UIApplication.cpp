...
BOOL CUIAppRegistryKey::WriteInt(LPCTSTR lpszSection, LPCTSTR lpszEntry, int nValue)
{
	...
	if (m_pszRegistryKey != NULL)
	{
		...
		if (hSecKey == NULL)
			return FALSE;
		LONG lResult = RegSetValueEx(hSecKey, lpszEntry, 0, REG_DWORD,
...
CDuiString CUIAppRegistryKey::ReadString(LPCTSTR lpszSection, LPCTSTR lpszEntry, LPCTSTR lpszDefault)
{
	...
	if (m_pszRegistryKey != NULL)
	{
		...
		if (lResult == ERROR_SUCCESS)
		{
            ...
            LPBYTE buffer = new BYTE[dwCount/sizeof(TCHAR)]{0};
			lResult = RegQueryValueEx(hSecKey, (LPTSTR)lpszEntry, NULL, &dwType,
				buffer, &dwCount);
            strValue = (TCHAR*)buffer;
		}
		RegCloseKey(hSecKey);
		if (lResult == ERROR_SUCCESS)
		{
...
BOOL CUIAppRegistryKey::ReadBinary(LPCTSTR lpszSection, LPCTSTR lpszEntry, LPBYTE* ppData, UINT* pBytes)
{
	...
	*pBytes = 0;
	...
	else
	{
		...
		CDuiString str = ReadString(lpszSection, lpszEntry, NULL);
		if (str.IsEmpty())
			return FALSE;
...
BOOL CUIAppRegistryKey::WriteBinary(LPCTSTR lpszSection, LPCTSTR lpszEntry, LPBYTE pData, UINT nBytes)
{
	...
	if (m_pszRegistryKey != NULL)
	{
		...
		if (hSecKey == NULL)
			return FALSE;
		lResult = RegSetValueEx(hSecKey, lpszEntry, 0, REG_BINARY,
			pData, nBytes);
		RegCloseKey(hSecKey);
		return lResult == ERROR_SUCCESS;
	}
