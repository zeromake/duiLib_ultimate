...
namespace DuiLib {
    ...
    UIImage::CDCCache::CDCCache() throw()
    {
        int iDC;
    ...
    HDC UIImage::CDCCache::GetDC() throw()
    {
        HDC hDC;
    ...
    void UIImage::CDCCache::ReleaseDC(_In_ HDC hDC) throw()
    {
        for( int iDC = 0; iDC < 4; iDC++ )
        {
            HDC hOldDC;
	...
	HDC UIImage::GetDC() const throw() {
        ASSERT(bitmap->GetBitmap() != NULL);
	    m_nDCRefCount++;
        if( m_hDC == NULL )
        {
            m_hDC = GetCDCCacheInstance()->GetDC();
            m_hOldBitmap = HBITMAP( ::SelectObject( m_hDC, bitmap->GetBitmap() ) );
        }
        return m_hDC;
    }
    ...
    void UIImage::ReleaseDC() const throw()
    {
        ...
        if( m_nDCRefCount == 0 )
        {
            ...
            ASSERT( hBitmap == bitmap->GetBitmap() );
    ...
    bool UIImage::Draw(
        ...
        return Draw(hDestDC, xDest, yDest, nDestWidth, nDestHeight, 0, 0, nWidth, nHeight);
    }
    bool UIImage::Draw(
        HDC hDestDC,
        int xDest,
    ...
    UIImage::~UIImage() throw()
    {
        Destroy();
    }
    void UIImage::Destroy() throw()
    {
        HBITMAP hBitmap;
    ...
    bool UIImage::AlphaBlend(
        HDC hDestDC,
        const RECT& rectDest,
        const RECT& rectSrc,
        BYTE bSrcAlpha,
        BYTE bBlendOp) const throw() {
    ...
    UIRect::UIRect(const RECT& srcRect) throw()
    {
        ::CopyRect(this, &srcRect);
    }
    
    int UIRect::Width() const throw()
    {
        return right - left;
    }

    int UIRect::Height() const throw()
    {
        return bottom - top;
    }
    void UIRect::operator=(_In_ const RECT& srcRect) throw()
    {
        ::CopyRect(this, &srcRect);
    }
    bool UIRect::IsRectEmpty() const throw()
    {
        return ::IsRectEmpty(this);
    }

    bool UIRect::IsRectNull() const throw()
    {
        return (left == 0 && right == 0 && top == 0 && bottom == 0);
    }
    UIRect::operator LPRECT() throw()
    {
        return this;
    }

    UIRect::operator LPCRECT() const throw()
    {
        return this;
    }
} // namespace DuiLib
