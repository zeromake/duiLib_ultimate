...
namespace DuiLib {
	...
	emRenderEngine CPaintManagerUI::m_emRenderEngine = DuiLib_Render_Gdi;
	...
	bool CPaintManagerUI::LoadPlugin(LPCTSTR pstrModuleName)
	{
		...
		if( hModule != NULL ) {
			...
			if( lpCreateControl != NULL ) {
				if( m_aPlugins.Find((void*)lpCreateControl) >= 0 ) return true;
				m_aPlugins.Add((void*)lpCreateControl);
				return true;
			}
		}
		return false;
		...
		case WM_MOUSEMOVE:
			{
				...
				if(bNeedDrag && m_bDragMode && wParam == MK_LBUTTON)
				{
					...
					HBITMAP hBitmap = (HBITMAP)OleDuplicateData(m_dragBitmap->GetBitmap(), fmtetc.cfFormat, 0);
