...
// 			}
    ...
    bool CControlUI::DrawImage(HBITMAP bitmap, int width, int height, bool bAlpha, BYTE uFade) {
        RECT rcBmpPart = {0};
        rcBmpPart.right = width;
		rcBmpPart.bottom = height;
        ASSERT(bitmap != NULL);
        GetManager()->Render()->DrawBitmap(
            bitmap,
            m_rcItem,
            m_rcPaint,
            rcBmpPart,
    ...
    bool CControlUI::DrawImageHDC(HDC dc, RECT rcBmpPart) {
        GetManager()->Render()->DrawBitmapHDC(
            dc,
            m_rcItem,
            m_rcPaint,
            rcBmpPart,
