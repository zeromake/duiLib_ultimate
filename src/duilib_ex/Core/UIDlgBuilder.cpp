...
// 				}
				...
				if( pManager->GetPaintWindow() && !pManager->IsInitWindowParameter())  //modify by liqs99
				{
					for( CXmlAttributeUI attr = root.first_attribute(); attr; attr=attr.next_attribute() ) 
					{
						...
						else if( _tcsicmp(pstrName, _T("renderengine")) == 0 ) 
						{
							...
							if( _tcsicmp(attr_s, _T("gdi")) == 0 )
								pManager->SetRenderEngineType(DuiLib_Render_Gdi);
							else if( _tcsicmp(attr_s, _T("gdiplus")) == 0 )
								pManager->SetRenderEngineType(DuiLib_Render_GdiPlus);
							else if( _tcsicmp(attr_s, _T("skia")) == 0 )
								pManager->SetRenderEngineType(DuiLib_Render_Skia);
							else if( _tcsicmp(attr_s, _T("d2d")) == 0 )
								pManager->SetRenderEngineType(DuiLib_Render_D2D);
						} 
    ...
    CDuiString CDialogBuilder::GetError() {
