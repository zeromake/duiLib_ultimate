...
{
	return (LPCTSTR)impxmlattr(_attr).name();
}

LPCTSTR CXmlAttributeUI::value() const
{
	return (LPCTSTR)impxmlattr(_attr).value();
}

LPCTSTR CXmlAttributeUI::as_string(LPCTSTR def) const
{
	return (LPCTSTR)impxmlattr(_attr).as_string((ui_pugi::char_t*)def);
}
...
{
	return impxmlattr(_attr).set_name((ui_pugi::char_t*)rhs);
}
