...
{    
    ...
    {
        UIImage* pImage = GetImageAt(i);
        pImage->Destroy();
        pImage->Release();
    }

    if(m_pOffScreenHDC != NULL)
    {
        ::DeleteDC(m_pOffScreenHDC->dc);
    }
}
...
bool CreateImage(
    ...
    hdc->height = nHeight;
    hdc->width = nWidth;
    auto oldBitmap = (HBITMAP)::SelectObject(hdc->dc, hBitmap);
    if (hdc->oldBitmap == NULL) {
        hdc->oldBitmap = oldBitmap;
    }
    return true;
}
...
bool CreateBitmap(
    ...
    return image->CreateImage(hBitmap, hasAlphaChannel);
}

void ReleaseDC(HDCImage* hdc) {
    ::SelectObject(hdc->dc, hdc->oldBitmap);
    hdc->oldBitmap = NULL;
}
...
void CImageBoxExUI::DoInit()
{
    ...
    if(m_pOffScreenHDC == NULL)
    {
        m_pOffScreenHDC = new HDCImage{};
        m_pOffScreenHDC->dc = ::CreateCompatibleDC(NULL);
        bCreate = CreateImage(m_pOffScreenHDC, 100, 100, 24, 0);
    }

    //初始动画
    SetAnimationType(CImageBoxExUI::kAnimationFade);
...
bool CImageBoxExUI::DoPaint(UIRender *pRender, const RECT& rcPaint, CControlUI* pStopControl)
{
    DrawImageHDC(
        m_pOffScreenHDC->dc,
        {0, 0, m_pOffScreenHDC->width, m_pOffScreenHDC->height});
    return true;
}
...
void CImageBoxExUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    if (m_pOffScreenHDC->width != (rc.right - rc.left) && m_pOffScreenHDC->height != (rc.bottom - rc.top))//add wenchangwei 2022-02-24
    {
        CreateImage(m_pOffScreenHDC, rc.right - rc.left, rc.bottom - rc.top, 24, 0);
    }
}
...
{
    if(m_strImagePath[m_strImagePath.GetLength()-1] != '\\') m_strImagePath += _T("\\");
    ...
    while (::FindNextFile(hFind, &FindData))
    {
        ...
        {
            auto *img = UIGlobal::CreateImage();
            CDuiString imgSrc = m_strImagePath + fileName;
            LPCTSTR _imgSrc = imgSrc;
            bool hr = img->LoadImage(_imgSrc);
            if(hr) {
...
UIImage* CImageBoxExUI::GetImageAt(int iIndex) const
{
    ASSERT(iIndex < (int)m_arrImages.size());
    return (UIImage*)(m_arrImages[iIndex]);
}
...
void CImageBoxExUI::AnimationRender_Alpha(DWORD dwTick)
{
    ...
    UIImage* pCurImage = GetImageAt(m_iIndex);
    UIImage* pNextImage = GetImageAt(GetNextFrameIndex());
    UIRect rcClient = GetPos();
    // DEST
...
void CImageBoxExUI::AnimationRender_Slide(DWORD dwTick)
{
    ...
    UIImage* pCurImage = GetImageAt(m_iIndex);
    UIImage* pNextImage = GetImageAt(GetNextFrameIndex());
    auto hOffScreenDC = m_pOffScreenHDC->dc;
    UIRect rcClient = GetPos();
    ::SetStretchBltMode(hOffScreenDC, COLORONCOLOR);
...
void CImageBoxExUI::AnimationRender_Blind(DWORD dwTick)
{
    ...
    UIImage* imageFit = UIGlobal::CreateImage();
    CreateBitmap(imageFit, rcClient.Width(), rcClient.Height(), 24, 0);
