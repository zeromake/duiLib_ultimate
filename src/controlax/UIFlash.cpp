...
#define __DEFINE_IID(name, l, w1, w2, b1, b2, b3, b4, b5, b6, b7, b8) \
        EXTERN_C const IID DECLSPEC_SELECTANY name \
                = { l, w1, w2, { b1, b2,  b3,  b4,  b5,  b6,  b7,  b8 } }

__DEFINE_IID(_IID_IShockwaveFlashEvents, 0xd27cdb6d, 0xae6d, 0x11cf, 0x96,0xb8,0x44,0x45,0x53,0x54,0x00,0x00);
__DEFINE_IID(_IID_IShockwaveFlash, 0xd27cdb6e, 0xae6d, 0x11cf, 0x96,0xb8,0x44,0x45,0x53,0x54,0x00,0x00);

#ifdef __MINGW32__
namespace _com_util {
    char* ConvertBSTRToString(wchar_t* s) {
        return DuiLib::w2a(s);
    }
}
#endif
...
namespace DuiLib
{
    CControlUI* CFlashUI::CreateControl() {
		return new CFlashUI;
	}
	...
	HRESULT STDMETHODCALLTYPE CFlashUI::QueryInterface( REFIID riid, void **ppvObject )
	{
		*ppvObject = NULL;
		...
		else if( riid == _IID_IShockwaveFlashEvents)
	...
	bool CFlashUI::DoCreateControl()
	{
		...
		GetControl(_IID_IShockwaveFlash,(LPVOID*)&m_pFlash);
	...
	HRESULT CFlashUI::RegisterEventHandler( BOOL inAdvise )
	{
		...
		if (FAILED(hr))
			return hr;
		hr=pCPC->FindConnectionPoint(_IID_IShockwaveFlashEvents,&pCP);
		if (FAILED(hr))
			return hr;
