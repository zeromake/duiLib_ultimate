...
#include <textserv.h>
	...
	HRESULT InitDefaultCharFormat(CRichEditUI* re, CHARFORMAT2W* pcf, HFONT hfont) 
	{
		...
		MultiByteToWideChar(CP_DEFAULT, 0, lf.lfFaceName, LF_FACESIZE, pcf->szFaceName, LF_FACESIZE) ;
#endif
	...
	BOOL CTxtWinHost::Init(CRichEditUI *re, const CREATESTRUCT *pcs)
	{
		...
		if(pcs && pcs->lpszName)
		{
			...
			::MultiByteToWideChar(CP_DEFAULT, 0, pcs->lpszName, -1, (LPWSTR)lpText, iLen) ;
	...
	HRESULT CTxtWinHost::TxGetPasswordChar(TCHAR *pch)
	{
		...
		*pch = chPasswordChar;
#else
		::WideCharToMultiByte(CP_DEFAULT, 0, &chPasswordChar, 1, pch, 1, NULL, NULL) ;
#endif
		return NOERROR;
	}
...
#else
		//need to thunk pcf->szFaceName to a standard char string.in this case it's easy because our thunk is also our copy
		MultiByteToWideChar(CP_DEFAULT, 0, lf.lfFaceName, LF_FACESIZE, cf.szFaceName, LF_FACESIZE) ;
#endif
	...
	void CRichEditUI::ReplaceSel(LPCTSTR lpszNewText, bool bCanUndo)
	{
		...
		LPWSTR lpText = new WCHAR[iLen + 1];
		::ZeroMemory(lpText, (iLen + 1) * sizeof(WCHAR));
		::MultiByteToWideChar(CP_DEFAULT, 0, lpszNewText, -1, (LPWSTR)lpText, iLen) ;
