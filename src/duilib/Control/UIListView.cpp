...
namespace DuiLib {
    ...
    {
        if (_tcsicmp(pstrName, _T("ListView")) == 0) return static_cast<CListViewUI*>(this);
        return CContainerUI::GetInterface(pstrName);
    }

    void CListViewUI::SetAdapter(ListViewAdapter* adapter)
    {
        m_Delegate = adapter;
        NeedUpdate();
    }
    ...
    void CListViewUI::SetScrollPos(SIZE szPos, bool bMsg)
    {
    ...
    void CListViewUI::SetPos(RECT rc, bool bNeedInvalidate) {
        ...
        const int width_required = m_Delegate->GetItemCount() == 0 ? 0 : m_HiddenItem->GetFixedWidth();
        ProcessScrollBar(szAvailable, width_required, m_total_height);

        const bool force_update = ProcessVisibleItems(item_view_count);
        UpdateSubviews(rc, force_update || m_data_updated);
    }

    bool CListViewUI::ProcessVisibleItems(int item_view_count) {
    ...
    void CListViewUI::ProcessScrollBar(SIZE szAvailable, int cxRequired, int cyRequired)
    {
        if (m_bScrollProcess)
            return;
    ...
    void CListViewUI::UpdateSubviews(RECT rc, bool force_update) {
        const int item_view_height = m_HiddenItem->GetFixedHeight();
        const int item_view_width = m_HiddenItem->GetFixedWidth();

        const int scroll_posY = (m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible()) ? m_ScrollY : 0;
        int first_visible_index = scroll_posY / item_view_height;
        const int itemview_pos_top = scroll_posY % item_view_height;
