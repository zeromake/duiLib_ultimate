...
namespace DuiLib {
	...
	bool CListUI::SelectMultiItem(int iIndex, bool bTakeFocus)
	{
		...
		if ((GetKeyState(VK_CONTROL) & 0x8000)) {
			...
			for (int index = iStart; index <= iEnd; index++) {
				...
				m_aSelItems.Add((LPVOID)(size_t)index);
			}
	...
	void CListUI::SelectAllItems()
	{
		for (int i = 0; i < GetCount(); ++i) {
			...
			int aIndex = m_aSelItems.Find((LPVOID)(size_t)i);
			if (aIndex < 0) {
	...
	void CListBodyUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		if (m_pOwner) {
			...
			if (pHeader != NULL && pHeader->GetCount() > 0) {
				cxNeeded = MAX(0, (int)(pHeader->EstimateSize(CDuiSize(rc.right - rc.left, rc.bottom - rc.top)).cx));
				if (m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible())
				{
	...
	DWORD CListTextElementUI::GetTextColor(int iIndex) const
	{
		...
		DWORD dwColor = (DWORD)(size_t)m_aTextColors.GetAt(iIndex);
		return dwColor;
	}
	...
	void CListTextElementUI::SetTextColor(int iIndex, DWORD dwTextColor)
	{
		...
		TListInfoUI* pInfo = m_pOwner->GetListInfo();
		if( iIndex < 0 || iIndex >= pInfo->nColumns ) return;
		while( m_aTextColors.GetSize() < pInfo->nColumns ) { m_aTextColors.Add((LPVOID)(size_t)pInfo->dwTextColor); }
		m_aTextColors.SetAt(iIndex, (LPVOID)(size_t)dwTextColor);
