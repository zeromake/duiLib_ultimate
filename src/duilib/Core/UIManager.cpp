...
namespace DuiLib {
	...
	void CPaintManagerUI::InitProcessDPIAware() {
#if(WINVER >= 0x0605)
		...
		case WM_MOUSEMOVE:
			{
				...
				if(bNeedDrag && m_bDragMode && wParam == MK_LBUTTON)
				{
					...
					//////////////////////////////////////
					HBITMAP hBitmap = (HBITMAP)OleDuplicateData(m_hDragBitmap, fmtetc.cfFormat, 0);
	...
	void DuiLib::CPaintManagerUI::SetDPI(int iDPI)
	{
        ...
        auto scaleRatio = scale2 / scale1;
		if (!::IsZoomed(GetPaintWindow())) {
			RECT rc = rcWnd;
            auto width = rcWnd.right - rcWnd.left;
            auto height = rcWnd.bottom - rcWnd.top;
			rc.right = rcWnd.left + (width * scaleRatio);
			rc.bottom = rcWnd.top + (height * scaleRatio);
			prcNewWindow = rc;
		}
		SetWindowPos(
            GetPaintWindow(),
