...
bool CMarkup::LoadFromMem(BYTE* pByte, DWORD dwSize, int encoding)
{
    ...
    if (encoding == XMLFILE_ENCODING_UTF8) {
        ...
        DWORD nWide = ::MultiByteToWideChar(1252, 0, (LPCSTR)pByte, dwSize, NULL, 0 );
        m_pstrXML = static_cast<LPTSTR>(malloc((nWide + 1)*sizeof(TCHAR)));
        ::MultiByteToWideChar(1252, 0, (LPCSTR)pByte, dwSize, m_pstrXML, nWide );
        m_pstrXML[nWide] = _T('\0');
    } else {
    ...
    if (encoding == XMLFILE_ENCODING_UTF8) {
        ...
        if ( dwSize >= 2 && ( ( pByte[0] == 0xFE && pByte[1] == 0xFF ) || ( pByte[0] == 0xFF && pByte[1] == 0xFE ) ) ) {
            ...
            DWORD nWide = ::WideCharToMultiByte(CP_DEFAULT, 0, (LPCWSTR)pByte, dwSize, NULL, 0, NULL, NULL);
            m_pstrXML = static_cast<LPTSTR>(malloc((nWide + 1)*sizeof(TCHAR)));
            ::WideCharToMultiByte(CP_DEFAULT, 0, (LPCWSTR)pByte, dwSize, m_pstrXML, nWide, NULL, NULL);
            m_pstrXML[nWide] = _T('\0');
            pByte -= 2;
        }
    } else {
