...
namespace DuiLib {
...
#ifdef UNICODE
						char* pwd = w2a((wchar_t*)sFilePwd.GetData());
						hz = OpenZip((TCHAR*)sFile.GetData(), pwd);
						if(pwd) delete[] pwd;
#else
						hz = OpenZip((TCHAR *)sFile.GetData(), (char *)sFilePwd.GetData());
#endif
					}
					if( hz == NULL ) break;
					ZIPENTRY ze; 
	...
	void CRenderEngine::DrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
	{
...
#else
			WCHAR * pcwszDest = a2w((char*)pstrText);
			if(pcwszDest != NULL)
			{
				if ((uStyle & DT_CALCRECT) != 0)
				{
		...
		else
		{
			...
			if (fonticonpos != -1) {
				...
				::DrawTextW(hDC, a2w((char*)pstrText), -1, &rc, uStyle);
			}
			::SelectObject(hDC, hOldFont);
		}
	}
	...
	void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, CDuiString* sLinks, int& nLinkRects, int iFont, UINT uStyle)
	{
		...
		int iLinkIndex = 0;
		int cyLine = pTm->tmHeight + pTm->tmExternalLeading + (int)(size_t)aPIndentArray.GetAt(aPIndentArray.GetSize() - 1);
		int cyMinHeight = 0;
		int cxMaxWidth = 0;
		POINT ptLinkStart = { 0 };
		bool bLineEnd = false;
		...
		while( *pstrText != _T('\0') ) {
			if( pt.x >= rc.right || *pstrText == _T('\n') || bLineEnd ) {
				...
				ptLinkStart = pt;
				cyLine = pTm->tmHeight + pTm->tmExternalLeading + (int)(size_t)aPIndentArray.GetAt(aPIndentArray.GetSize() - 1);
					...
					case _T('a'):  // Link
						{
							...
							//}
							aColorArray.Add((LPVOID)(size_t)clrColor);
							...
							if( pFontInfo->bUnderline == false ) {
								...
								cyLine = MAX(cyLine, pTm->tmHeight + pTm->tmExternalLeading + (int)(size_t)aPIndentArray.GetAt(aPIndentArray.GetSize() - 1));
							}
							ptLinkStart = pt;
							bInLink = true;
						}
					...
					case _T('p'):  // Paragraph
						{
							...
							aPIndentArray.Add((LPVOID)(size_t)cyLineExtra);
				...
				case _T('c'):
					{
						...
						DWORD clrColor = dwTextColor;
						if( aColorArray.GetSize() > 0 ) clrColor = (int)(size_t)aColorArray.GetAt(aColorArray.GetSize() - 1);
						::SetTextColor(hDC, RGB(GetBValue(clrColor), GetGValue(clrColor), GetRValue(clrColor)));
					}
					break;
				case _T('p'):
	...
	HBITMAP CRenderEngine::CreateARGB32Bitmap(HDC hDC, int cx, int cy, BYTE** pBits)
	{
		...
		HBITMAP hBitmap = CreateDIBSection (hDC, lpbiSrc, DIB_RGB_COLORS, (void **)pBits, NULL, 0);
		delete [] lpbiSrc;
		return hBitmap;
	}
